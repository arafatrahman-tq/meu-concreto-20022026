...
export async function getWhatsappConfig(companyId: number) {
  ...
  // ─── Merge logic: trust the global instance if the company doesn't have its own valid connection ───
  // A company has its own connection if it has a phone number AND it is not just inheriting the global one's values.
  const hasOwnConnection = !!(
    companySetting?.phoneNumber
    && companySetting.phoneNumber !== globalSetting?.phoneNumber
  )
  ...
  return {
    ...
    // Prefer company-specific fields if they appear to be customized (not the default or if manually set)
    // If not, fall back to global connection details entirely.
    apiUrl: hasOwnConnection
      ? companySetting?.apiUrl || 'http://localhost:3025'
      : globalSetting?.apiUrl || 'http://localhost:3025',
    apiKey: hasOwnConnection
      ? companySetting?.apiKey || null
      : globalSetting?.apiKey || null,
    phoneNumber: instanceName,
    isConnected: hasOwnConnection
