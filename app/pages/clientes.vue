...
type ActivityFilter = 'all' | 'buyers' | 'prospects' | 'inactive'

// ─────────────────────────────────────────────
// Data
// ─────────────────────────────────────────────
const { data: customersData, pending: loadingCustomers } = await useFetch(
  () => `/api/customers?companyId=${companyId.value}`
)
...
const customers = computed<Customer[]>(
  () =>
    ((customersData.value as Record<string, unknown>)
      ?.customers as Customer[]) ?? []
)
const allQuotes = computed<DrawerQuote[]>(
  () =>
    ((quotesData.value as Record<string, unknown>)?.quotes as DrawerQuote[])
    ?? []
)
const allSales = computed<DrawerSale[]>(
  () =>
    ((salesData.value as Record<string, unknown>)?.sales as DrawerSale[]) ?? []
)
...
// Avatar hue (deterministic per name)
const AVATAR_COLORS = [
  'bg-green-500',
  'bg-blue-500',
  'bg-purple-500',
...
const stats = computed(() => {
  ...
  const newThisMonth = all.filter(
    c => c.lastActivityAt >= startOfMonth
  ).length
...
const fetchCep = async () => {
  const cleanCep = newCustomerForm.cep.replace(/\D/g, '')
...
const confirmNewCustomer = async () => {
  ...
  try {
    const displayAddress = [
      newCustomerForm.street?.trim(),
      newCustomerForm.number?.trim()
    ]
      .filter(Boolean)
      .join(', ')

    // Ensure customer is registered as a "Company" record if document is provided
    if (newCustomerForm.document.trim()) {
      try {
        await $fetch('/api/companies', {
          ...
          body: {
            ...
            zip: newCustomerForm.cep.trim() || undefined
          }
        })
      } catch (e: any) {
        // Skip 409 Conflict (Already registered) as it's not an error we need to stop for
        if (e.statusCode !== 409) {
          console.error('Failed to register customer to database', e)
        }
      }
    }

    // Build specialized address string for compatibility
    const parts = []
    if (displayAddress) parts.push(displayAddress)
    if (newCustomerForm.complement?.trim()) {
      parts.push(newCustomerForm.complement.trim())
    }
    if (newCustomerForm.neighborhood?.trim()) {
      parts.push(newCustomerForm.neighborhood.trim())
    }
    if (newCustomerForm.city?.trim()) {
      parts.push(
        `${newCustomerForm.city.trim()}/${newCustomerForm.state?.trim() || '—'}`
      )
    }
    if (newCustomerForm.cep?.trim()) {
      parts.push(`CEP: ${newCustomerForm.cep.trim()}`)
    }

    const fullAddress = parts.join(' - ')

    isNewDrawerOpen.value = false
    navigateTo({
      path: '/orcamentos',
      query: {
        customerName: newCustomerForm.name.trim(),
        customerDocument: newCustomerForm.document.trim() || undefined,
        customerPhone: newCustomerForm.phone.trim() || undefined,
...
<template>
  <div class="p-6 lg:p-8 space-y-6">
    ...
    ══════════════════════════════════════════ -->
    <USlideover
      v-model:open="isNewDrawerOpen"
      title="Novo Cliente"
      description="Preencha os dados e inicie um orçamento"
      side="right"
      :ui="{ content: 'w-full max-w-md', footer: 'p-0 block' }"
    >
      <template #body>
        <div class="p-6 space-y-6">
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <UFormField label="CPF / CNPJ">
                <UInput
                  v-model="newCustomerForm.document"
                  placeholder="000.000..."
                  icon="i-heroicons-identification"
                  class="w-full"
                  @update:model-value="
                    (v) => (newCustomerForm.document = maskDocument(v))
                  "
                />
              </UFormField>
              ...
              <UFormField label="Telefone">
                <UInput
                  ...
                  class="w-full"
                  @update:model-value="
                    (v) => (newCustomerForm.phone = maskPhone(v))
                  "
                />
              </UFormField>
            </div>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <UFormField label="CEP">
                <UInput
                  v-model="newCustomerForm.cep"
                  placeholder="00000-000"
                  icon="i-heroicons-map"
                  class="w-full"
                  :loading="loadingCep"
                  @update:model-value="
                    (v) => (newCustomerForm.cep = maskCep(v))
                  "
                  @blur="fetchCep"
                >
                  <template #trailing>
                    <UButton
                      v-if="newCustomerForm.cep.length >= 8"
                      color="neutral"
                      variant="link"
                      icon="i-heroicons-magnifying-glass"
                      size="xs"
            ...
            <div class="grid grid-cols-3 gap-4">
              <UFormField label="Nº">
                <UInput
                  v-model="newCustomerForm.number"
                  placeholder="123"
                  class="w-full"
                />
              </UFormField>
              ...
              >
                <UInput
                  v-model="newCustomerForm.neighborhood"
                  placeholder="Centro"
                  class="w-full"
                />
              </UFormField>
            ...
            <UFormField label="Complemento">
              <UInput
                v-model="newCustomerForm.complement"
          ...
          <UAlert
            ...
            title="Como funciona"
            description="Ao confirmar, você será direcionado para criar um orçamento com os dados deste cliente pré-preenchidos."
          />
        </div>
      </template>
      ...
      <template #body>
        ...
        >
          ...
          >
            ...
            <div
              v-if="!selectedCustomer.phone && !selectedCustomer.address"
          ...
          <div class="grid grid-cols-3 gap-3">
            <div
              class="rounded-xl bg-white dark:bg-zinc-900 ring-1 ring-zinc-200 dark:ring-zinc-800 p-3 text-center shadow-xs"
            >
          ...
          <div class="space-y-4">
            ...
            >
              ...
              >
                ...
                {{ tab.label }}
                <span
                  class="ml-1 text-[10px] font-black bg-zinc-100 dark:bg-zinc-800 text-zinc-500 rounded-full px-1.5 py-0.5"
                >
                  {{
                    tab.value === "quotes"
            ...
            >
              <div
                ...
                class="flex flex-col items-center py-12 text-zinc-400 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border border-dashed border-zinc-200 dark:border-zinc-700"
              >
                <UIcon
