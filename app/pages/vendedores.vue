...
const totalPages = computed(() => Math.ceil(filteredSellers.value.length / pageSize.value))
...
watch(
  ...
  (val) => {
    ...
    let masked = d
    if (d.length > 9) masked = d.replace(/(\d{3})(\d{3})(\d{3})(\d{0,2})/, '$1.$2.$3-$4')
    else if (d.length > 6) masked = d.replace(/(\d{3})(\d{3})(\d{0,3})/, '$1.$2.$3')
...
const handleSave = async () => {
  ...
  if (form.commissionRate < 0 || form.commissionRate > 100) {
    toast.add({ title: 'Campos inválidos', description: 'A comissão deve estar entre 0% e 100%.', color: 'warning', icon: 'i-heroicons-exclamation-triangle' })
    return
  }
  ...
  try {
    ...
    if (isEditing.value && editingId.value) {
      await $fetch(`/api/sellers/${editingId.value}`, { method: 'PUT', body: payload })
      toast.add({
      ...
      await $fetch('/api/sellers', { method: 'POST', body: { companyId: companyId.value, ...payload } })
      toast.add({
    ...
    toast.add({
      ...
      description: err?.data?.statusMessage ?? err?.message ?? 'Erro ao salvar.',
      color: 'error',
...
const toggleActive = async (s: Seller) => {
  try {
    await $fetch(`/api/sellers/${s.id}`, { method: 'PUT', body: { active: !s.active } })
    toast.add({
...
<template>
  <div class="p-6 lg:p-8 space-y-6">
    ...
    <UCard>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-zinc-100 dark:border-zinc-800">
              <th class="text-left px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400">
                Vendedor
              </th>
              <th class="text-left px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400 hidden md:table-cell">
                CPF
              </th>
              <th class="text-left px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400 hidden lg:table-cell">
                Telefone
              </th>
              <th class="text-center px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400">
                Comissão
              </th>
              <th class="text-left px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400">
                Status
              </th>
              <th class="text-right px-4 py-3 text-xs font-black uppercase tracking-widest text-zinc-400">
                Ações
              </th>
            </tr>
          </thead>
            ...
            >
              ...
              <td class="px-4 py-3.5">
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 rounded-xl bg-primary-100 dark:bg-primary-500/10 flex items-center justify-center shrink-0">
                    <span class="text-sm font-black text-primary-600 dark:text-primary-400">
                      {{ sellerInitials(s.name) }}
                    </span>
                  </div>
                  <div class="min-w-0">
                    <p class="font-bold text-zinc-900 dark:text-white truncate max-w-36 sm:max-w-52">
                      {{ s.name }}
                    </p>
                    <p
                      v-if="s.email"
              ...
              <!-- CPF -->
              <td class="px-4 py-3.5 text-xs text-zinc-500 font-mono whitespace-nowrap hidden md:table-cell">
              ...
              <td class="px-4 py-3.5">
                <button @click="toggleActive(s)">
                  <UBadge
                    ...
                    variant="soft"
                    size="sm"
                    :icon="s.active ? 'i-heroicons-check-circle' : 'i-heroicons-eye-slash'"
      ...
      <!-- Pagination -->
      <template
        v-if="filteredSellers.length > pageSize"
        #footer
      >
